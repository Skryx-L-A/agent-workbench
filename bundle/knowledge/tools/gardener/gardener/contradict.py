...
def escalation_reason(a: Note, b: Note) -> str | None:
    ...
    """
    hay = (a.text + "\n" + b.text).lower()
    hit = next((t for t in ESCALATE_TERMS if t in hay), None)
    if hit:
...
def write_review_queue(vault: Path, findings: list[dict], dry_run: bool) -> Path:
    ...
    if QUEUE_SECTION_START in existing and QUEUE_SECTION_END in existing:
        head = existing[:existing.index(QUEUE_SECTION_START)]
        tail = existing[existing.index(QUEUE_SECTION_END) + len(QUEUE_SECTION_END):]
